...
The model predicts `log1p(RN60)` and is fitted with Smooth-L1 loss. The
...
|---|---|---|
| `schema` | scalar string | exactly `patch_mlp_prepared_npz_v1` |
...
```bash
python -m patch_mlp.workflow validate \
...
```bash
python -m patch_mlp.workflow select-epoch \
  --input prepared_2023.npz \
...
```bash
python -m patch_mlp.workflow fit \
  --input prepared_2023.npz \
  --output patch_mlp_seed21040.pt \
  --epochs 39 \
  --seed 21040 \
  --device cuda

python -m patch_mlp.workflow predict \
  --input prepared_2024_2025.npz \
  --checkpoint patch_mlp_seed21040.pt \
  --output predictions_seed21040.npz \
  --device cuda
```

For canonical dense input, prediction output contains
