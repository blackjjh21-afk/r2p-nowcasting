"""Natural-prevalence objective used for final Patch MLP fitting."""
...
def importance_corrected_smooth_l1(
    ...
    *,
    ...
    """Estimate unweighted natural-risk Smooth-L1 after stratified sampling.
    ...
    ordinary Smooth-L1 on log1p(RN60); stratification changes coverage and
    variance but not the target risk.
    """

    if prediction_log.shape != target_log.shape or target_mm.shape != target_log.shape:
        raise ValueError("prediction, log target, and millimetre target shapes differ")
    per_tuple = F.smooth_l1_loss(
        prediction_log, target_log, beta=float(beta), reduction="none"
    )
