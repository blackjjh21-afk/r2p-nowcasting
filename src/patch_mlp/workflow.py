"""Portable prepared-data workflow for the field-first Patch MLP.
...
gauge or radar files.  See :mod:`patch_mlp`'s README for the NPZ schema.
"""
...
PREPARED_SCHEMA = "patch_mlp_prepared_npz_v1"
CHECKPOINT_SCHEMA = "patch_mlp_checkpoint_v1"
PREDICTION_SCHEMA = "patch_mlp_predictions_v1"
SELECTION_SCHEMA = "patch_mlp_month_station_oof_v1"
PATCH_SHAPE = (6, 3, 3)
AUX_DIM = 39
STATION_FOLDS = (0, 1, 2)
    ...
    *,
    ...
    for start in range(0, len(order), batch_size):
        ...
        if recipe == TARGET_STRATIFIED_V1:
            ...
            loss = importance_corrected_smooth_l1(
                prediction,
                target_log,
                target_mm,
            ...
            loss = F.smooth_l1_loss(prediction, target_log, beta=1.0)
...
def predict_mm(
    ...
    *,
    ...
    for start in range(0, len(selected), batch_size):
        ...
        patch = torch.from_numpy(data.patches[rows]).to(device)
        aux = torch.from_numpy(auxiliary[rows]).to(device)
        value = millimetres_from_log_prediction(model(patch, aux))
...
def load_checkpoint(path: Path, device: torch.device) -> tuple[PatchMLP, dict]:
    # Patch MLP checkpoints contain only tensors and primitive metadata, so the
    # restricted loader avoids Python pickle code execution.
    payload = torch.load(path, map_location="cpu", weights_only=True)
    if not isinstance(payload, dict) or payload.get("schema") != CHECKPOINT_SCHEMA:
        raise RuntimeError("unsupported Patch MLP checkpoint")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    select.add_argument("--seed", type=int, default=22500)
    select.add_argument("--validation-batch-size", type=int, default=65_536)
    add_training_arguments(select)

    fit = commands.add_parser("fit", help="fit one final Patch MLP")
